...
object AppVersionUtils {
            ...
            try {
                ...
                if (isVersionOlder(current, latest)) {
                    withContext(Dispatchers.Main) {
                        showUpdateDialog(activity, latest, appUrl)
                    }
                }
            } catch (e: Exception) {
    ...
    private fun showUpdateDialog(activity: Activity, latestVersion: String, appUrl: String? = null) {
        val context = activity.applicationContext
        AlertDialog.Builder(activity)
            .setTitle(context.getString(R.string.need_update))
            .setMessage("최신 버전 ($latestVersion)으로 앱을 업데이트해주세요.")
